...
import {
  Language as RouteLanguage,
  languages as routeLanguages,
} from '~/routes';

/**
 * An array of supported language codes (re-exported from the routes module).
 */
export const languages = routeLanguages;

/**
 * A type representing a supported language code  (re-exported from the routes module).
 */
export type Language = RouteLanguage;
 ...
 */
export function getNamespaces(routeModules: RouteModules): Namespace {
...
function isRouteHandle(value?: unknown): value is RouteHandle {
  ...
  return isObject && 'i18nNamespaces' in value;
}
